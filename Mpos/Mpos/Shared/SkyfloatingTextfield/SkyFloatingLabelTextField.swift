...
open class SkyFloatingLabelTextField: UITextField { // swiftlint:disable:this type_body_length
    ...
    fileprivate func createTitleLabel() {
        ...
        if Device.SCREEN_HEIGHT > 320
        {
            titleLabel.font = isIpad ? UIFont.systemFont(ofSize: 13) : UIFont.systemFont(ofSize: 9)
        }
        else
        {
            titleLabel.font = isIpad ? UIFont.systemFont(ofSize: 12) : UIFont.systemFont(ofSize: 8)
        }        
    ...
    open func titleHeight() -> CGFloat {
        if let strlbl = titleLabel.text
        {
            let rect = CGSize(width: titleLabel.bounds.width, height: CGFloat.greatestFiniteMagnitude)
            let labelSize = strlbl.boundingRect(with: rect, options: .usesLineFragmentOrigin, attributes: [NSAttributedString.Key.font: titleLabel.font], context: nil)
            
            let numberOfLines:Int =  Int(ceil(CGFloat(labelSize.height) / titleLabel.font.lineHeight))
